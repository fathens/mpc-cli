...
mod tests {
    ...
    fn test_share_protocol() {
        ...
        // テスト用のNTildeを使用
        let ntilde_alice = NTildei::generate_for_test()[0].clone();
        let ntilde_bob = NTildei::generate_for_test()[1].clone();

        println!("Aliceの初期化処理を開始...");
        let start = Instant::now();
        let (ca, proof_alice) =
            alice_init::<Secp256k1>(&pk_alice, &get_test_small_int(), &ntilde_alice).unwrap();
        println!(
            "Aliceの初期化に{}秒かかりました",
            start.elapsed().as_secs_f64()
        );
    ...
    fn test_share_protocol_wc() {
        ...
        // テスト用のNTildeを使用
        let ntilde_alice = NTildei::generate_for_test()[2].clone();
        let ntilde_bob = NTildei::generate_for_test()[3].clone();
